...
const JetVizPage: React.FC = () => {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
        ...
        setHasAnalyzed(true);
        setIsSuccessDialogOpen(true); // Trigger success dialog after analysis
      ...
      <header className="relative pt-12 pb-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center z-10">
            ...
            <div className="max-w-6xl mx-auto relative group mb-12 animate-fade-in-up">
                {/* Frame */}
                <div 
                  className="relative w-full aspect-[16/10] md:aspect-[21/9] rounded-3xl overflow-hidden border-4 border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)] cursor-ew-resize select-none"
                  ref={containerRef}
                            ...
                            <div className="bg-white/5 border border-white/10 p-2 rounded-[2rem] backdrop-blur-xl max-w-2xl mx-auto shadow-2xl">
                                <form onSubmit={handleAnalyze} className="flex flex-col sm:flex-row gap-2">
                                    ...
                                    >
                                        {isAnalyzing ? (
                                            <>
                                                <Loader2 className="w-5 h-5 border-2 border-slate-400 border-t-black rounded-full animate-spin">
                                                </Loader2>
                                                Analyzing...
                                            </>
                                        ) : (
            ...
            {/* Success Dialog for Analysis Completion */}
            <SuccessDialog
                isOpen={isSuccessDialogOpen}
                onClose={() => setIsSuccessDialogOpen(false)}
                title="Analysis Complete!"
                message={`Your website audit for ${new URL(url).hostname} is ready. Review the detailed scores below and book a call to discuss your results.`}
