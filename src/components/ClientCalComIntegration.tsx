      ...
      {isConnected ? (
        <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl space-y-2">
          ...
          <p className="text-sm text-emerald-700">
            Booking is enabled via your Cal.com event type.
          </p>
          {status?.default_event_type_id ? (
            <p className="text-xs text-slate-600">Event Type ID: <span className="font-mono font-semibold">{status.default_event_type_id}</span></p>
          ) : (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800 flex items-start gap-2">
              <Info className="w-4 h-4 mt-0.5" />
              <div>
                <p className="font-semibold">One more step: set an Event Type ID</p>
                <p className="text-xs mt-1">Your Cal.com connection is ready, but booking requires a default Event Type ID. Ask an admin to set it for this client.</p>
              </div>
            </div>
          )}
          {status?.updated_at && (
        ...
        <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
          ...
          {needsReauth ? (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800 flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs mt-1">
                  Please reconnect Cal.com so we can securely store a refresh token.
                </p>
                {status?.last_error && (
            ...
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800 flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs mt-1">
                  Cal.com is our preferred booking provider for Retell AI automation.
                </p>
              </div>
            </div>
          )}
          ...
          <p className="text-sm text-slate-600">
            Connect your Cal.com account so the AI agent can check availability and book meetings.
          </p>
          <button
