...
const ClientLayout: React.FC<ClientLayoutProps> = ({ children }) => {
    ...
    return (
      ...
      >
        <item.icon className={`w-4 h-4 flex-shrink-0 ${
          isActive ? 'text-white' : isDark ? 'text-slate-500 group-hover:text-slate-300' : 'text-slate-400 group-hover:text-slate-600'
        }`} />
      ...
      <div className={`px-4 py-3 border-b ${isDark ? 'border-slate-700' : 'border-slate-100'} ${isMobile ? 'mt-14' : ''}`}>
        <div className={`flex items-center gap-3 p-2.5 rounded-xl border ${
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-[11px] font-medium ${isDark ? 'text-indigo-400' : 'text-indigo-600'}`}>Client Portal</p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'dark bg-slate-900' : 'bg-[#F4F6FA]'}`}>
      ...
      {/* Mobile Header */}
      <div className={`fixed left-0 right-0 z-40 border-b px-4 h-14 flex items-center justify-between md:hidden shadow-sm ${
        isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'
      } ${isAdminView ? 'top-[36px]' : 'top-0'}`}>
      ...
      <div className="flex">
        {/* Desktop Sidebar */}
        <aside className={`hidden md:flex w-60 border-r fixed left-0 h-screen flex-col shadow-sm z-30 ${
          isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'
        } ${isAdminView ? 'top-[36px]' : 'top-0'}`}>
          <SidebarContent />
        </aside>
