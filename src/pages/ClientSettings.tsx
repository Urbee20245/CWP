...
import { Settings, ArrowRight, Loader2, Phone, Zap, Info, AlertTriangle, Clock, CheckCircle2 } from 'lucide-react';
...
const ClientSettings: React.FC = () => {
  ...
  const getA2PStatusDisplay = () => {
    switch (a2pStatus) {
      case 'approved':
        return { label: 'Approved', color: 'text-emerald-600', bg: 'bg-emerald-50', icon: CheckCircle2 };
      case 'pending_approval':
      case 'submitted':
        return { label: 'Pending Approval', color: 'text-blue-600', bg: 'bg-blue-50', icon: Clock };
      case 'rejected':
        return { label: 'Needs Attention', color: 'text-red-600', bg: 'bg-red-50', icon: AlertTriangle };
      default:
        return { label: 'Not Started', color: 'text-amber-600', bg: 'bg-amber-50', icon: Info };
    }
  };
  ...
  return (
    <ClientLayout>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <p className="text-slate-600 mb-10 max-w-2xl">Manage your third-party integrations and AI service configurations.</p>
          ...
          <div className="bg-white rounded-2xl shadow-xl border-2 border-indigo-600 overflow-hidden relative group">
            ...
            <div className="p-8">
              ...
              <p className="text-sm text-slate-600 leading-relaxed mb-6">
                Let our team handle the technical heavy lifting. We will provision your business number, configure the AI logic,
                and ensure network compliance for you.
              </p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {a2pStatus === 'approved' && voiceStatus === 'active' && (
                      <p className="text-xs text-emerald-700 mt-1 font-semibold">AI Call Handling is fully active and operational.</p>
                    )}
                    {a2pStatus === 'approved' && voiceStatus !== 'active' && (
                      <p className="text-xs text-emerald-700 mt-1 font-semibold">Verification complete. AI Call Handling will activate shortly.</p>
                    )}
                    {a2pStatus !== 'approved' && (
                      <p className="text-xs text-slate-500 mt-1">You must submit your business details to start the verification process.</p>
                    )}
                  </div>
                </div>
              </div>
