...
import { Globe, Loader2, Eye, Save, Lock, CheckCircle, AlertTriangle, ExternalLink } from 'lucide-react';

// Resolve a dot-path string to its value in an object
...
// Human-readable label from a dot-path
...
const ClientWebsite: React.FC = () => {
  ...
  // Local edits: field_path → current value
  const [edits, setEdits] = useState<Record<string, string>>({});

  // Load client record → then load brief
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (briefData) {
        ...
        if (briefData.website_json) {
          for (const section of briefData.website_json.page_structure || []) {
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              {brief.client_slug && (
                <span className="text-sm text-slate-400 font-mono">/site/{brief.client_slug}</span>
              )}
            </div>

            {/* Editable sections */}
            <div className="space-y-4">
              {brief.website_json.page_structure.map((section, sIdx) => {
                const hasEditable = section.editable_fields.length > 0;
                return (
                  <div key={sIdx} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="px-5 py-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                      ...
                      <span className="text-xs text-slate-400 font-mono">{section.variant}</span>
                    </div>
                    <div className="p-5">
                      {hasEditable ? (
                        <div className="space-y-4">
                          {section.editable_fields.map((path) => {
                        ...
                        <div className="flex items-center gap-2 text-slate-400 text-sm py-2">
                          ...
                          This section's content is managed by your design team.
                        </div>
                      )}
                    </div>
                  </div>
