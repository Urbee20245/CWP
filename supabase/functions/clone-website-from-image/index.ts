...
// ─── System prompt for website design analysis + generation ──────────────────

const SYSTEM_INSTRUCTION = `You are an expert web designer. You will be given a screenshot of a website and business information.
Your task is to:
1. Analyse the visual design of the screenshot (color palette, layout style, typography feel, section types visible)
2. Generate a complete multi-page website JSON for the given business that mirrors the visual style of the screenshot
...
serve(async (req) => {
  ...
  if (!business_name) return errorResponse('business_name is required.', 400);
  if (!industry) return errorResponse('industry is required.', 400);
  if (!services_offered) return errorResponse('services_offered is required.', 400);
  ...
  console.log('[clone-website-from-image] Starting for client_id=' + client_id);

  // Get existing record for slug preservation
  const { data: existing } = await db.from('website_briefs')
    .select('client_slug, slug')
    .eq('client_id', client_id)
    .maybeSingle();

  const client_slug = await makeUniqueSlug(
    db,
    existing?.client_slug || slugify(business_name),
    client_id,
  );
  ...
  const { error: upsertError } = await db.from('website_briefs').upsert({
    ...
    art_direction: 'Cloned from uploaded screenshot',
    generation_status: 'generating',
    generation_error: null,
  }, { onConflict: 'client_id' });

  if (upsertError) {
    console.error('[clone-website-from-image] Upsert failed: ' + upsertError.message);
  ...
  try {
    ...
    const userPrompt = `Clone the visual design of the attached website screenshot for this business:
...
Location: ${location}
Tone: ${tone}
${primary_color ? `Preferred color (override if needed): ${primary_color}` : ''}

PAGES TO GENERATE:
${pagesGuide}

Analyse the screenshot's visual design (colors, layout, typography, sections) and apply that aesthetic to this business.
All copy must be written specifically for ${business_name} — do NOT copy text from the screenshot.
Return complete JSON now.`;
    ...
    try {
      ...
      console.error('[clone-website-from-image] JSON parse failed. Raw:', rawText.slice(0, 400));
      await db.from('website_briefs').update({
        generation_status: 'error',
        generation_error: 'AI returned invalid JSON.',
      }).eq('client_id', client_id);
    ...
    if (!websiteJson.global || !Array.isArray(websiteJson.pages) || !websiteJson.pages.length) {
      await db.from('website_briefs').update({
        generation_status: 'error',
        generation_error: 'AI response missing required fields.',
      }).eq('client_id', client_id);
      return errorResponse('AI response missing required fields.', 500);
    }

    // Apply overrides and fallbacks
    websiteJson.global.phone = websiteJson.global.phone || '';
    websiteJson.global.address = websiteJson.global.address || location;
    if (primary_color) websiteJson.global.primary_color = primary_color;
    websiteJson.global.primary_color = websiteJson.global.primary_color || '#4F46E5';
    ...
    if (saveError) {
      console.error('[clone-website-from-image] Save failed: ' + saveError.message);
      return errorResponse('Failed to save website: ' + saveError.message, 500);
    }

    console.log('[clone-website-from-image] SUCCESS slug=' + client_slug + ' pages=' + websiteJson.pages.length);
    return jsonResponse({ success: true, client_slug, website_json: websiteJson });

  } catch (error: any) {
    console.error('[clone-website-from-image] Error: ' + error.message);
    await db.from('website_briefs').update({
      generation_status: 'error',
