...
serve(async (req) => {
  ...
  // Read user Authorization header and create a user-scoped client for ownership checks
  ...
  try {
    ...
    if (!user) {
      return new Response(JSON.stringify({ error: 'Unauthorized: Invalid JWT' }), { status: 401, headers: corsHeaders });
    }
    ...
    const encryptedSid = config.account_sid_encrypted as string;
    const maskedSid = encryptedSid ? encryptedSid.slice(-4) : '****';
